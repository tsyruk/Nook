...
function showConfirm(msg, onConfirm) {
  ...
  document.getElementById('confirm-yes').onclick = () => { overlay.classList.remove('open'); onConfirm(); };
...
function showToast(msg, type = 'ok') {
  ...
  // shrink font for very long messages so they don't overflow
  t.style.fontSize = msg.length > 40 ? '8px' : '9px';
...
/**
 * Generates a unique customer ID using the Web Crypto API.
 * Previously used Math.random() with only 9,000 possible values — replaced
 * to eliminate collision risk and produce a cryptographically random ID.
 */
function genId() {
  return 'NOOK-' + crypto.randomUUID().replace(/-/g, '').slice(0, 8).toUpperCase();
}

/**
 * Sanitizes a string for safe insertion into innerHTML.
 * Prevents XSS from user-supplied content (e.g. staff notes in history).
 */
...
function renderHistList(elId, c) {
  ...
  if (!h.length) {
    document.getElementById(elId).innerHTML =
      '<div style="font-size:9px;color:rgba(0,255,136,.2);padding:8px;text-align:center;">NO ACTIVITY YET</div>';
    return;
  }
  document.getElementById(elId).innerHTML = h.map(e => `
    <div class="hist-item">
      ...
      <div class="hist-text">${H_LBL[e.type] || e.type}
        ${e.note ? `<div style="font-size:8px;color:rgba(0,229,255,.5);margin-top:1px;">${sanitize(e.note)}</div>` : ''}
      </div>
...
function renderRewardChips(elId, c, showBtn, fnName) {
  ...
  document.getElementById(elId).innerHTML = all.map(r => {
    ...
    return `<div class="reward-chip ${cls}">
      <div>
        <div style="font-size:9px;color:${col};margin-bottom:2px;">☕ FREE COFFEE</div>
        <div style="font-size:8px;color:rgba(0,229,255,.5);">
          ${expired ? 'EXPIRED' : 'EXPIRES'} ${r.expires}${!expired ? ' (' + left + 'd LEFT)' : ''}
          ${r.note ? ' · ' + sanitize(r.note) : ''}
        </div>
      </div>
