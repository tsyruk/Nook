  ...
  if (staffPin.length === 4) {
    setTimeout(async () => {
      ...
      try {
        ...
        showBusy(false);
        staffPin = ''; updateStaffPinUI();
        // Bug fix: was incorrectly writing to 'apin-err' (admin PIN error element).
        // Staff PIN screen uses 'spin-err'.
        document.getElementById('spin-err').textContent = e.message || '!! WRONG PIN !!';
...
function renderCustomerList() {
  ...
  if (!sorted.length) {
    el.innerHTML = '<div style="font-size:9px;color:rgba(255,0,255,.3);padding:14px;text-align:center;">NO PLAYERS FOUND</div>';
    return;
  }
  el.innerHTML = sorted.map(c => {
    ...
    return `<div class="c-row" onclick="openStaffDetail('${c.id}')">
      ...
      <div style="flex:1;">
        <div style="font-size:10px;color:var(--g);margin-bottom:3px;">${bday}${c.name}</div>
        <div style="font-size:9px;color:rgba(0,229,255,.5);">${c.phone}</div>
      </div>
      <div style="text-align:right;">
        <div style="font-size:9px;color:var(--y);">☕ ${c.stamps}/10</div>
        ${pr > 0 ? `<div style="font-size:9px;color:var(--g);margin-top:2px;">★ ${pr} REWARD${pr>1?'S':''}</div>` : ''}
      </div>
    </div>`;
  }).join('');
}
...
async function sdAddStamp() {
  ...
  lastStampTime = now;
  if (!staffToken || isTokenExpired(staffToken)) { clearStaffToken(); showToast('SESSION EXPIRED — PLEASE LOG IN AGAIN', 'err'); go('s-staff-login'); return; }
  showBusy(true);
  try {
...
async function sdRedeemById(rid) {
  if (!staffToken || isTokenExpired(staffToken)) { clearStaffToken(); showToast('SESSION EXPIRED', 'err'); go('s-staff-login'); return; }
  showBusy(true);
  try {
